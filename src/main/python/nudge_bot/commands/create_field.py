...
@click.option('--field-type',     help='The type of field to create', type=click.Choice(['Text','MultiSelect','Select']), default='Select', required=True)
@click.option('--allowed-value',     help='The value for the field (can be specified multpile times)', multiple=True)
@click.pass_obj
def list_fields(nudge_client:NudgeClient,field_scope, field_name, field_type, allowed_value):
